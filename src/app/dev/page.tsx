  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-3">
        <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-100">Technical comparison</h2>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          ...
          {COMPARISON_ROWS.map((row, i) => (
            <div
              ...
              className={`grid grid-cols-3 gap-4 px-4 py-3 text-sm ${
                i < COMPARISON_ROWS.length - 1 ? 'border-b border-zinc-100 dark:border-zinc-800' : ''
              }`}
            >
      ...
      <Section title="How the integrations work">
        ...
        <p className="mt-2">
          ...
          the full OAuth 2.0 Authorization Code Flow. The user authorises once via the Google consent screen;
          the server exchanges the auth code for a refresh token and stores it in the client&apos;s localStorage.
          Every subsequent API call (write or read) calls <Code>refreshAccessToken()</Code> server-side first —
          the client never sees an access token. The refresh token persists indefinitely until the user
          revokes access from their Google account settings.
        </p>
        <p className="mt-2">
          The OAuth callback route (<Code>/api/auth/google/callback</Code>) exchanges the code for tokens,
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-1">
          <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-100">Developer setup</h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
        ...
        {/* Sheets API setup */}
        <div className="flex flex-col gap-4">
          <h3 className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">Sheets API — one-time Google Cloud setup</h3>
          <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
            <div className="flex flex-col gap-5">
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                This setup is done once by the developer. After it&apos;s complete, any user can connect
                their Google account without any further configuration.
              </p>

              <Step n={1}>
                <span>Go to <span className="font-medium text-zinc-800 dark:text-zinc-200">console.cloud.google.com</span> and create or select a project.</span>
                <ul className="ml-4 list-disc space-y-1">
                  <li><strong>APIs &amp; Services → Library</strong> → search for <strong>Google Sheets API</strong> → Enable</li>
                </ul>
              </Step>
              ...
              <Step n={3}>
                <span>Add the credentials to <Code>.env.local</Code> (local) and your hosting platform (production):</span>
                ...
                <span>
                  Generate <Code>ENCRYPTION_KEY</Code> with:{' '}
              ...
              <Step n={4}>
                <span>If your Google Cloud project is in <strong>Testing</strong> mode, add test users:</span>
                <ul className="ml-4 list-disc space-y-1">
                  <li><strong>OAuth consent screen → Test users → Add users</strong></li>
      ...
      <Section title="Credentials and security model">
        <p>
          <Code>GOOGLE_CLIENT_ID</Code>, <Code>GOOGLE_CLIENT_SECRET</Code>, and <Code>ENCRYPTION_KEY</Code> live
          in environment variables and are only used server-side. They are never sent to the client.
        </p>
        <p className="mt-2">
          After the OAuth flow completes, the server encrypts the refresh token with AES-256-GCM using
          <Code>ENCRYPTION_KEY</Code> before sending it to the client. The encrypted blob is stored in{' '}
          <Code>localStorage</Code>. When the client calls <Code>/api/sheets</Code>, it sends the blob;
          the server decrypts it, exchanges it for a short-lived access token, makes the Google Sheets
