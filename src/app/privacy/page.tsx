  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-4 dark:border-amber-800/50 dark:bg-amber-900/20">
        <p className="text-sm font-semibold text-amber-800 dark:text-amber-400">App currently in testing mode</p>
        <p className="mt-1 text-sm text-amber-700 dark:text-amber-500">
          The Google Sheets integration is currently limited to approved test accounts. If you&apos;d like
          to try it, get in touch and I&apos;ll add your Google account to the list.
        </p>
        <a
      ...
      <Section title="Google account access (Sheets API)">
        <p>
          When you use the <strong className="text-zinc-700 dark:text-zinc-300">Sheets API</strong> integration,
          the app requests access to Google Sheets with the following scope:
        </p>
        <ul className="mt-2 ml-4 list-disc space-y-1">
          <li><code className="rounded bg-zinc-100 px-1 font-mono text-xs text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">https://www.googleapis.com/auth/spreadsheets</code> — read and write access to Google Sheets, used solely to append and fetch expense rows in the spreadsheet you specify.</li>
          <li><code className="rounded bg-zinc-100 px-1 font-mono text-xs text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">email</code> — your email address, displayed in the app to confirm which account is connected.</li>
        </ul>
        <p className="mt-2">
        ...
        <p className="mt-2">
          ...
          <code className="rounded bg-zinc-100 px-1 font-mono text-xs text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">localStorage</code>.
      ...
      <Section title="No server-side data storage">
        <p>
          This app has no database. No expense data, no credentials, and no personal information
          are persisted on any server. Each API request is stateless — credentials are passed in
          the request body, used for that request only, and discarded.
        </p>
      </Section>
      ...
      <Section title="Revoking access">
        <p>
          To disconnect the app from your Google account, either click <strong className="text-zinc-700 dark:text-zinc-300">Disconnect</strong> in
          Settings → Connect, or go to your Google account at{' '}
          <a href="https://myaccount.google.com/permissions" target="_blank" rel="noopener noreferrer" className="underline underline-offset-2">
