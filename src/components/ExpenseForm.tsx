  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    const pushSheets = async (): Promise<PushOutcome> => {
      ...
      try {
        ...
        if (!navigator.onLine) { enqueueSync(expense.id, 'sheets'); return { ok: false, offline: true } }
    ...
    if (allOk) {
      ...
      setStatus({ type: 'error', message: `Expense saved locally. Sync failed: ${apiErrors || 'Connection error.'}` })
    }
